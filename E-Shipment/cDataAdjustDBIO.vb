Public Class cDataAdjustDBIO
    ...
    '----------------------------------------------------------------------
    Public Function AdjustExist(ByVal KeyString As String, ByRef Tran As System.Data.OleDb.OleDbTransaction) As Boolean
    ...
    '　戻値：取得レコード数
    '----------------------------------------------------------------------
    Public Function getAdjust(ByRef ParAdjust() As cStructureLib.sAdjust,
                              ByVal KeyFromAdjustCode As Long,
                              ByVal KeyToAdjustCode As Long,
                              ByVal KeyAdjustDate As String,
                              ByVal KeyAdjustClass1 As String,
                              ByVal KeyAdjustClass2 As String,
        ...
        StrSelect = "SELECT 精算データ.精算コード AS 精算コード, " &
                            "精算データ.精算区分 AS 精算区分, " &
                            "精算データ.精算日 AS 精算日, " &
                            "精算データ.金額 AS 金額, " &
                            "精算データ.勘定科目コード AS 勘定科目コード, " &
                            "勘定科目マスタ.勘定科目名称 AS 勘定科目名称, " &
                            "勘定科目マスタ.税区分コード AS 税区分コード, " &
                            "税区分マスタ.税区分名称 AS 税区分名称, " &
                            "精算データ.補助勘定科目コード AS 補助勘定科目コード, " &
                            "勘定科目補助マスタ.補助勘定科目名称 AS 補助勘定科目名称, " &
                            "精算データ.レジ入金金額 AS レジ入金金額, " &
                            "精算データ.既レジ入金金額 AS 既レジ入金金額, " &
                            "精算データ.D10000円 AS D10000円, " &
                            "精算データ.D5000円 AS D5000円, " &
                            "精算データ.D1000円 AS D1000円, " &
                            "精算データ.D500円 AS D500円, " &
                            "精算データ.D100円 AS D100円, " &
                            "精算データ.D50円 AS D50円, " &
                            "精算データ.D10円 AS D10円, " &
                            "精算データ.D5円 AS D5円, " &
                            "精算データ.D1円 AS D1円, " &
                            "精算データ.金庫入金金額 AS 金庫入金金額, " &
                            "精算データ.K10000円 AS K10000円, " &
                            "精算データ.K5000円 AS K5000円, " &
                            "精算データ.K1000円 AS K1000円, " &
                            "精算データ.K500円 AS K500円, " &
                            "精算データ.K100円 AS K100円, " &
                            "精算データ.K50円 AS K50円, " &
                            "精算データ.K10円 AS K10円, " &
                            "精算データ.K5円 AS K5円, " &
                            "精算データ.K1円 AS K1円, " &
                            "精算データ.精算担当者コード AS 精算担当者コード, " &
                            "精算データ.登録日 AS 登録日, " &
                            "精算データ.登録時間 AS 登録時間, " &
                            "精算データ.最終更新日 AS 最終更新日, " &
                            "精算データ.最終更新時間 AS 最終更新時間 " &
                    "FROM ((勘定科目マスタ INNER JOIN 税区分マスタ " &
                    "ON 勘定科目マスタ.税区分コード = 税区分マスタ.税区分コード) " &
                    "INNER JOIN 勘定科目補助マスタ " &
                    "ON 勘定科目マスタ.勘定科目コード = 勘定科目補助マスタ.勘定科目コード) " &
                    "INNER JOIN 精算データ " &
                    "ON (精算データ.補助勘定科目コード = 勘定科目補助マスタ.補助勘定科目コード) " &
    ...
    Public Function getAdjust2(ByRef ParAdjust() As cStructureLib.sAdjust,
                              ByVal KeyFromAdjustCode As Long,
                              ByVal KeyToAdjustCode As Long,
                              ByVal KeyAdjustDate As String,
                              ByVal KeyAdjustClass1 As String,
        ...
        StrSelect = "SELECT 精算データ.精算コード AS 精算コード, " &
                    ...
                    "FROM ((精算データ " &
                    "LEFT JOIN 勘定科目マスタ " &
                    "  ON 精算データ.勘定科目コード = 勘定科目マスタ.勘定科目コード) " &
                    "LEFT JOIN 勘定科目補助マスタ " &
                    "  ON 精算データ.勘定科目コード = 勘定科目補助マスタ.勘定科目コード " &
                    " AND 精算データ.補助勘定科目コード = 勘定科目補助マスタ.補助勘定科目コード) " &
                    "LEFT JOIN 税区分マスタ " &
                    "  ON 勘定科目マスタ.税区分コード = 税区分マスタ.税区分コード " &
                    "WHERE 精算コード >= " & KeyFromAdjustCode & " " &
                    "AND (精算区分 = """ & KeyAdjustClass1 & """ " &
    ...
    '----------------------------------------------------------------------
    Public Function getAdjust3(ByRef ParAdjust() As cStructureLib.sAdjust,
                              ByVal KeyFromAdjustCode As Long,
                              ByVal KeyToAdjustCode As Long,
                              ByVal KeyAdjustDate As String,
                              ByVal KeyAdjustClass1 As String,
    ...
    '　戻値：取得レコード数
    '----------------------------------------------------------------------
    Public Function getBetweenAdjust(ByRef ParAdjust() As cStructureLib.sAdjust,
                              ByVal KeyFromAdjustDate As String,
                              ByVal KeyToAdjustDate As String,
                              ByRef Tran As System.Data.OleDb.OleDbTransaction) As Long
        ...
        StrSelect = "SELECT 精算データ.精算コード AS 精算コード, " &
                            ...
                            "精算データ.最終更新日 AS 最終更新日, " &
                            "精算データ.最終更新時間 AS 最終更新時間 " &
                    "FROM ((精算データ LEFT JOIN 勘定科目マスタ ON 精算データ.勘定科目コード = 勘定科目マスタ.勘定科目コード) " &
                    "LEFT JOIN 税区分マスタ ON 勘定科目マスタ.税区分コード = 税区分マスタ.税区分コード) " &
                    "LEFT JOIN 勘定科目補助マスタ " &
                    "ON (精算データ.勘定科目コード = 勘定科目補助マスタ.勘定科目コード) " &
                    "AND (精算データ.補助勘定科目コード = 勘定科目補助マスタ.補助勘定科目コード) " &
                    "WHERE (精算日 >= """ & KeyFromAdjustDate & """ AND 精算日 <= """ & KeyToAdjustDate & """ )" &
    ...
    Public Function getBeforeAdjust(ByVal KeyAdjustCode As Long, ByRef Tran As System.Data.OleDb.OleDbTransaction) As Long
        ...
        Try
            StrSelect = "SELECT * FROM 精算データ WHERE 精算コード < @AdjustCode " &
                        "AND 精算区分 = ""精算"" " &
                        "ORDER BY 精算コード DESC"
    ...
    Public Function insertAdjust(ByRef parAdjust As cStructureLib.sAdjust, ByRef Tran As System.Data.OleDb.OleDbTransaction) As Boolean
        ...
        Try
            ...
            'SQL文の設定
            strInsertAdjust = "INSERT INTO 精算データ" &
            "( 精算コード, 精算区分, 精算日, 金額, 勘定科目コード, 補助勘定科目コード, " &
            "レジ入金金額, 既レジ入金金額, D10000円, D5000円, D1000円, D500円, D100円, D50円, D10円, D5円, D1円, " &
            "金庫入金金額, K10000円, K5000円, K1000円, K500円, K100円, K50円, K10円, K5円, K1円, " &
            "精算担当者コード, 登録日, 登録時間, 最終更新日, 最終更新時間 ) " &
            "VALUES ( " &
                parAdjust.sAdjustCode & ", " &
                """" & parAdjust.sAdjustClass & """, " &
                "FORMAT(NOW, ""yyyy/mm/dd""), " &
                parAdjust.sTotalPrice & ", " &
                parAdjust.sAccountCode & ", " &
                parAdjust.sSubAccountCode & ", " &
                parAdjust.sDTotalPrice & ", " &
                parAdjust.sAlreadyDTotalPrice & ", " &
                parAdjust.sD10000Yen & ", " &
                parAdjust.sD5000Yen & ", " &
                parAdjust.sD1000Yen & ", " &
                parAdjust.sD500Yen & ", " &
                parAdjust.sD100Yen & ", " &
                ...
                parAdjust.sD5Yen & ", " &
                parAdjust.sD1Yen & ", " &
                parAdjust.sKTotalPrice & ", " &
                parAdjust.sK10000Yen & ", " &
                parAdjust.sK5000Yen & ", " &
                parAdjust.sK1000Yen & ", " &
                parAdjust.sK500Yen & ", " &
                parAdjust.sK100Yen & ", " &
    ...
    End Function
    Public Function readMaxAdjustCode(ByVal KeyAdjustClass As String,
                                        ByVal KeyCloseDate As String,
                                        ByVal KeyOperetor As String,
                                        ByRef Tran As System.Data.OleDb.OleDbTransaction) As Long
        Dim strSelect As String
        Dim i As Long
        Dim pc As Integer
    ...
    Public Function deleteAdjust(ByVal KetAdjustCode As Long, ByRef Tran As System.Data.OleDb.OleDbTransaction) As Long
        ...
        Try
            ...
            strDeleteAdjust = "DELETE * FROM 精算データ " &
    ...
    Public Function updateAdjustCode(ByVal KetFromAdjustCode As Long, ByVal KetToAdjustCode As Long, ByRef Tran As System.Data.OleDb.OleDbTransaction) As Boolean
        ...
        Try
            ...
            strUpdate = "UPDATE 精算データ SET " &
                            "精算コード=" & KetToAdjustCode & ", " &
