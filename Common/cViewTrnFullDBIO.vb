Public Class cViewTrnFullDBIO
    ...
    Public Function getTrnFull(ByRef parTrnFull() As cStructureLib.sViewTrnFull, _
        ...
        Try
            ...
            strSelect = "SELECT  " &
                            "日次取引データ.取引コード AS 取引コード, " &
                            "日次取引明細データ.取引明細コード AS 取引明細コード, " &
                            "日次取引データ.取引区分 AS 取引区分, " &
                            "日次取引データ.チャネルコード AS チャネルコード, " &
                            "チャネルマスタ.チャネル名称 AS チャネル名称, " &
                            "日次取引データ.支払方法コード AS 支払方法コード, " &
                            "支払方法マスタ.支払方法名称 AS 支払方法名称, " &
                            "日次取引データ.取引税抜商品金額 AS 取引合計税抜商品金額, " &
                            "日次取引データ.送料 AS 送料, " &
                            "日次取引データ.手数料 AS 手数料, " &
                            "日次取引データ.値引き AS 合計値引き, " &
                            "日次取引データ.ポイント値引き AS 合計ポイント値引き, " &
                            "日次取引データ.取引税抜金額 AS 取引合計税抜金額, " &
                            "日次取引データ.取引消費税額 AS 取引合計消費税額, " &
                            "日次取引データ.取引税込金額 AS 取引合計税込金額, " &
                            "日次取引データ.出荷コード AS 出荷コード, " &
                            "日次取引データ.会員コード AS 会員コード, " &
                            "日次取引データ.性別 AS 性別, " &
                            "日次取引データ.年代 AS 年代, " &
                            "日次取引データ.天気 AS 天気, " &
                            "日次取引データ.日次締め日 AS 日次締め日, " &
                            "日次取引データ.月次締め日 AS 月次締め日, " &
                            "日次取引データ.取引担当者コード AS 取引担当者コード, " &
                            "日次取引明細データ.売上状態 AS 売上状態, " &
                            "日次取引明細データ.売上明細区分 AS 売上明細区分, " &
                            ...
                            "日次取引明細データ.定価 AS 定価, " &
                            "日次取引明細データ.仕入単価 AS 仕入単価, " &
                            "日次取引明細データ.取引商品単価 AS 取引商品単価, " &
                            "日次取引明細データ.取引数量 AS 取引数量, " &
                            "日次取引明細データ.取引税抜商品金額 AS 取引税抜商品金額, " &
                            "日次取引明細データ.値引き AS 単品値引き, " &
                            "日次取引明細データ.ポイント値引き AS 単品ポイント値引き, " &
                            "日次取引明細データ.取引税抜金額 AS 取引税抜金額, " &
                            "日次取引明細データ.取引消費税額 AS 取引消費税額, " &
                            "日次取引明細データ.取引軽減消費税額 AS 取引軽減消費税額, " &
                            "日次取引明細データ.軽減税率 AS 軽減税率, " &
                            "日次取引明細データ.取引税込金額 AS 取引税込金額 " &
                        "FROM ( " &
            ...
            While pDataReader.Read()
                ...
                If IsDBNull(pDataReader("取引軽減消費税額")) = True Then
                    parTrnFull(i).sReducedTaxRatePrice = 0
                Else
                    parTrnFull(i).sReducedTaxRatePrice = pDataReader("取引軽減消費税額")
                End If
                '軽減税率
                If IsDBNull(pDataReader("軽減税率")) = True Then
                    parTrnFull(i).sReducedTaxRate = 0
                Else
                    parTrnFull(i).sReducedTaxRate = pDataReader("軽減税率")
                End If
    ...
    Public Function getFinTrnFull(ByRef parFinTrnFull() As cStructureLib.sViewFinTrnFull, _
        ...
        Try
            ...
            strSelect = "SELECT " &
                            ...
                            "日次取引明細データ.部門コード AS 部門コード, " &
                            "部門マスタ.部門名称 AS 部門名称, " &
                            "Sum(日次取引明細データ.取引税抜商品金額) AS 取引税抜商品金額, " &
                            "Sum(日次取引明細データ.値引き) AS 単品値引き, " &
                            "Sum(日次取引明細データ.ポイント値引き) AS 単品ポイント値引き, " &
                            "Sum(日次取引明細データ.取引税抜金額) AS 取引税抜金額, " &
                            "Sum(日次取引明細データ.取引消費税額) AS 取引消費税額, " &
                            "Sum(日次取引明細データ.取引軽減消費税額) AS 取引軽減消費税額, " &
                            "Sum(日次取引明細データ.軽減税率) AS 軽減税率, " &
                            "Sum(日次取引明細データ.取引税込金額) AS 取引税込金額 " &
                        "FROM " &
                            "((日次取引データ LEFT JOIN 日次取引明細データ " &
                                "ON 日次取引データ.取引コード = 日次取引明細データ.取引コード) " &
                            "LEFT JOIN 支払方法マスタ " &
            ...
            While pDataReader.Read()
                ...
                '取引軽減消費税額
                If IsDBNull(pDataReader("取引軽減消費税額")) = True Then
                    parFinTrnFull(i).sReducedTaxRatePrice = 0
                Else
                    parFinTrnFull(i).sReducedTaxRatePrice = CLng(pDataReader("取引軽減消費税額"))
                End If
                '軽減税率
                If IsDBNull(pDataReader("軽減税率")) = True Then
                    parFinTrnFull(i).sReducedTaxRate = 0
                Else
                    parFinTrnFull(i).sReducedTaxRate = CLng(pDataReader("軽減税率"))
                End If
                '2020,1,14 A.Komita 追加 To
